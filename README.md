...
```
serMidi --match "device_info.device_path == '/dev/ttyUSB0'"
```
...
```
serMidi --list --match "device_info.usb_description.find('USB2.0') != -1"
```

Convert usb serial device with usb product id 29987 to a virtual midi device with the name "MidiFoo"
```
serMidi --name "MidiFoo" --match "device_info.usb_pid == 29987"
```

Set the baudrate
```
serMidi --baud-rate 96000 --name "MidiFoo" --match "device_info.usb_pid == 29987"
```
...
1. Clone this repository `git clone https://github.com/aidevsurya/SerMidi.git`
2. cd into the folder `cd SerMidi`
3. install python dependencies with `pip install -r dependencies.txt`
4. run the tool with `python main.py`
5. Optionally, copy it to a folder in your path, for example `cp main.py /usr/local/bin/serMidi`

# Usage
```
usage: serMidi [-h] [--name NAME] [--baud-rate BAUD_RATE]
